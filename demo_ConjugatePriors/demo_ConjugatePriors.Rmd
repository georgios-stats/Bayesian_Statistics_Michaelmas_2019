...
#  pdf<- gamma(a+b)/ ( gamma(a)*gamma(b)*gamma(a+b+n) ) *choose(n, x) *gamma(a+x) *gamma(b+n-x)
#  return (pdf)  
...
```{r}
StudentT_pdf<-function(x,mu,lambda,alpha) {
  y <- (x-mu)/sqrt(1/lambda) ;
  pdf <- dt(y,df=alpha)
  return (pdf)  
}
```
...
NormalModel_PDF_meanvar_post<-function(mu,vu,n,sufstat,mu0,n0,a0,b0) {
  ...
  b_hat <- b0+0.5*n*x_var+0.5*n0*n*(mu0-x_mean)/(n0+n) ;
  
  mu_hat <- (n0*mu0+n*x_mean)/(n0+n) ;
  
  vu_hat <- vu/n_hat
  
  pdf <- dnorm( (mu-mu_hat)/sqrt(vu_hat),
                  mean=0, 
                    sd=1
                )*dgamma(
                  1/vu,
...
#  b0=1.5;
  ...
  plot(x_vu, 
       ...
       ylab = expression(paste(pi(sigma,x[1:n]), " and " ,pi(sigma))), 
       col="blue" ,
      cex.lab=fntsz, 
      cex.axis=fntsz, 
      cex.main=fntsz, 
  ...
  contour(x = x_mu,
         ...
         z = muvu_prior_pdf,
         col="green",
         plot.title = title(main = "Prior joint PDF",
                            xlab = expression(mu),
  ...
  contour(x = x_mu,
         ...
         z = muvu_post_pdf,
         col="blue",
         plot.title = title(main = "Posterior joint PDF",
                            xlab = expression(mu),
                            ylab = expression(sigma^2),
                            cex.lab=fntsz, 
                            cex.axis=fntsz, 
