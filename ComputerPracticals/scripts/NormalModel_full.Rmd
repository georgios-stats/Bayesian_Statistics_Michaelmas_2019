...
To exptract the sample from the output of `stan{rstan}` you need to use the function `extract{rstan}`, as in ***step 3***.
...
<!-- Compute the MC approximate of the predictive PDF and CDF and compare it with the exact ones.   -->
...
<!-- Compute the MC approximate of $\text{E}(y_{n+1}|y_{1:n})$, and compare it with the exact one which is   -->

<!-- \[ -->
<!-- \text{E}(y_{n+1}|y_{1:n} ) = \mu_n -->
<!-- \] -->
...
<!--      ylab = "CDF") -->
<!-- x_plot <- seq( from = min(z), to = max(z), length.out = 100) -->
<!-- y_plot <- studentT_cdf(x_plot , mu_n , lam_n*a_n/b_n, 2*a_n) -->

<!-- x_plot <- seq( from = min(z), to = max(z), length.out = 100) -->
<!-- y_plot <- studentT_cdf(x_plot , mu_n , lam_n*b_n/((lam_n+1)*a_n), 2*a_n) -->
<!-- lines(x_plot,  -->
<!--       y_plot, -->
<!--       type = 'l', -->
<!--       col = 'red') -->
...
<!-- Compute the MC approximate of the posterior mean and variace of $y_{n+1}$, decoted as  $\text{E}_p (y_{n+1}|y_{1:n})$, and $$\text{Var}_p (y_{n+1}|y_{1:n})$$ and compare it with the exact ones which are    -->

<!-- + $\text{E}_p (y_{n+1}|y_{1:n}) = \mu_n$ -->

<!-- + $\text{Var}_p (y_{n+1}|y_{1:n}) = \lambda_{n}  b_{n} / a_{n}$ -->

<!-- ### ... addressing {-}    -->

<!-- The Monte Carlo approximation for $\text{E}_p (y_{n+1}|y_{1:n})$ is   -->
...
<!-- The exact value for $\text{E}_p (y_{n+1}|y_{1:n})$ is   -->
...
<!-- The Monte Carlo approximation for $\text{Var}_p (y_{n+1}|y_{1:n})$ is   -->
...
<!-- The exact value for $\text{Var}_p (y_{n+1}|y_{1:n})$ is   -->

<!-- ```{r} -->
<!-- lam_n /(lam_n+1)* b_n / a_n *  ( (2*a_n) / (2*a_n-2) ) -->
<!-- ``` -->
