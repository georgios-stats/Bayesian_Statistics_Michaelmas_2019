...
Write the JAGS program implementing the hierarchical model above, in order to generate a sample of size $N=100000$ from the posterior distribution $$\beta^{(j)}\sim\pi(\beta|y_{1:n})\  ,\  \  j=1,...,N.$$.
...
n.thin = 10^3     # the thining (improving) the sample quality
...
Extract the sample drawn from the posterior distribution $$\beta^{(j)}\sim\pi(\beta|y_{1:n})\  \  j=1,...,N$$ and print the trace plot of the sample. 
...
\end{align*}
or maybe we could include the iteration as well 
